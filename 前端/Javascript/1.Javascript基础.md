...
### 可选链式运算符
可选链式运算符 ( `?.`) 是 JavaScript 中的一个实用工具，它允许你安全地访问对象属性或调用方法，而无需担心它们是否存在。它就像一张安全网，用于处理可能缺少部分的对象。
```js
const person = {
  name: "Alice",
  age: 30
...
```
通过使用可选链式运算符，我们告诉 JavaScript 仅当对象（或之前的值`?.`）存在且不是 null`null`或 false时才继续执行操作`undefined`。

如果前面的值`?.`是 null`null`或 False `undefined`，JavaScript 将返回 null`undefined`而不是尝试继续执行操作并抛出错误。
### 运算符的优先级
运算符的优先级如下：（优先级从高到低）
...
## 对象
### 对象简介
在 JavaScript 中，对象是一种基本数据结构，它允许你存储和组织相关的数据和功能。你可以把物体想象成一个容器，里面装着各种信息，就像文件柜里装着不同的文件夹和文件一样。这些信息被称为属性，它们由名称（或键）和值组成。
```js
const exampleObject = {
  propertyName: value,
}
...
对象具有**特征**（属性）和**行为**（方法）。
在 JavaScript 中，访问对象属性主要有两种方法：点号表示法和方括号表示法。
点号表示法是访问对象属性最常用、最直接的方式。以下是点号表示法的基本语法：
```js
objectName.propertyName
```
方括号表示法允许您使用方括号内的字符串来访问对象属性。以下是方括号表示法的使用方法：
...
移除属性的方法是使用带有剩余参数的解构赋值。这种方法实际上并不会删除属性，而是创建一个不包含指定属性的新对象：
...
`Object.hasOwn()`方法
用于检查对象是否拥有某个属性（非继承属性）的现代方法。可以将其视为升级版的、更安全的版本`hasOwnProperty()`。语法如下`Object.hasOwn(object, propertyName)`：将对象作为第一个参数传递，属性名称作为第二个参数传递。
```js
const person = {
  name: "Alice",
  age: 30
...
```
需要理解的重要一点是，它只检查属性是否**存在**，而不关心属性的值。这意味着`true`即使属性值为  `0`、`false`或null，它仍然会返回 true 。
```js
const user = {
  username: "coder123",
  score: 0,
...
检查`undefined`
检查属性是否为真`undefined`。这种方法虽然有用，但也存在一些局限性。
```js
const car = {
  brand: "Toyota",
  model: "Corolla",
...
### 对象解构
对象解构是 JavaScript 中的一项强大功能，它允许你从对象中提取值，并以更简洁、更易读的方式将它们赋值给变量。
### 内置对象 String
...
```
JSON 使用键值对来存储信息，每对键值之间用逗号分隔。每个键都必须用双引号括起来，否则会出错。
...
```
可以使用方括号表示法来访问 JSON 对象中的信息。以下是访问数组的示例`list of courses`：
...
在 JSON 字符串和 JavaScript 对象之间进行转换
...
```
## 数组
**定义**：JavaScript 数组是一个有序的值集合，每个值都由一个数字索引标识。JavaScript 数组中的值可以是不同的数据类型，包括数字、字符串、布尔值、对象，甚至其他数组。数组在内存中是连续的，这意味着所有元素都存储在单个连续的内存块中，从而可以高效地进行索引，并通过索引快速访问元素。
...
要访问数组中的元素，需要引用数组，并在其后加上方括号内的索引号。JavaScript 数组的索引从 0 开始，这意味着第一个元素的索引为 0，第二个元素的索引为 1，依此类推。如果尝试访问数组中不存在的索引，JavaScript 将返回 `undefined`。
```js
...
```
#### **更新数组中的元素**
要更新数组中的元素，可以使用赋值运算符 ( `=`) 将新值赋给指定索引处的元素。
```js
...
对于连续的数组，使用 length 可以获取到数组的长度（元素的个数）；对于非连续的数组，length 的值会大于元素的个数。因此，尽量不要创建非连续的数组。
#### 修改数组的长度
...
| pop()                                                | 此方法用于从数组中删除最后一个元素，并返回被删除的元素。如果数组为空，则返回值为`undefined` | 会改变原数组                                                                                                                                                                  |
| unshift()                                            | 在数组**最前面**插入一个或多个元素，返回结果为新数组的**长度**                 | 会改变原数组                                                                                                                                                                  |
| shift()                                              | 此方法用于从数组中删除第一个元素并返回该删除的元素。如果数组为空，则返回值为`undefined`   | 会改变原数组                                                                                                                                                                  |
|                                                      |                                                     |                                                                                                                                                                         |
| splice(startIndex,<br>itemsToRemove,<br>item1,item2) | 从数组中**删除**指定的一个或多个元素，返回结果为**被删除元素组成的新数组**           | `startIndex`指定开始修改数组的索引，而是`itemsToRemove`一个可选参数，用于指示要删除的元素数量。如果`itemsToRemove`省略，`splice()`则会删除从索引 0`startIndex`到数组末尾的所有元素。后续参数（例如`item1`，、`item2`等等）是从起始索引开始添加到数组中的元素。 |
...
```
`splice(1, 0, "yellow", "purple")`从索引 开始`1`，移除`0`元素，并插入`yellow`和`purple`。第二个参数（`0`）表示插入前不移除任何元素。也可以使用`splice()`同时移除和添加元素：
```js
...
此方法返回一个新数组，其中包含原始数组一部分的浅拷贝，拷贝范围由起始索引和结束索引指定。新数组包含指向与原始数组相同元素的引用（而非重复元素）。这意味着，如果元素是基本类型（例如数字或字符串），则复制的是值；但如果元素是对象或数组，则复制的是引用，而不是对象本身。
...
| 方法                         | 描述                                                        | 备注                                                                                                               |
| -------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| indexOf(element,fromIndex) | 从前往后索引，检索一个数组中是否含有指定的元素。如果找不到该元素，则返回空值`-1`。               | `element``value` 表示要在数组中查找的值，`start``fromIndex`参数是搜索的起始位置。`start``fromIndex`参数是可选的。如果`fromIndex`未提供，则搜索从数组的开头开始。 |
| lastIndexOf(value)         | 从后往前索引，检索一个数组中是否含有指定的元素                                   |                                                                                                                  |
| includes(item,fromIndex)   | 数组中是否包含指定的内容，此方法返回一个布尔值：如果数组包含指定的元素，则返回 true ；否则返回 false。 | 注：该方法在处理字符串时区分大小写。可选的第二个参数，用于指定在数组中开始搜索的位置。`includes()`使用严格相等比较（`===`），这意味着它可以区分不同类型的数据。                         |
...
```
### 二维数组
二维数组本质上是一个数组的数组。它用于表示具有天然网格状结构的数据，例如棋盘、电子表格或图像中的像素。要访问二维数组中的元素，需要两个索引：一个用于行索引，一个用于列索引。
```js
const chessboard = [
    ['R', 'N', 'B', 'Q', 'K', 'B', 'N', 'R'],
    ['P', 'P', 'P', 'P', 'P', 'P', 'P', 'P'],
...
对象解构是 JavaScript 中的一项强大功能，它允许你从对象中提取值，并以更简洁、更易读的方式将它们赋值给变量。
它是 ES6（ECMAScript 2015）规范的一部分，并且已成为许多 JavaScript 开发人员必不可少的工具。
对象解构的核心在于将对象中的值解包到不同的变量中。这样，你无需逐个访问对象属性，而是可以在一条语句中提取多个属性。这可以使你的代码更简洁、更高效。
#### 语法


可以使用对象解构的方式进行赋值。举例如下：
...
对象解构的一个强大之处在于，你可以将提取出的值赋给具有不同名称的变量。这在处理属性名称可能与现有变量冲突的对象，或者当你想要使用不同的名称时，尤其有用：
```js
...
```
在这种情况下，我们提取`name`属性并将其赋值给一个名为 的变量`personName`，并对`age`和执行相同的操作`personAge`。

对象解构还允许你设置默认值。如果要解构的对象中不存在某个属性，你可以指定一个备用值：
```js
let person = { name: "Alice", age: 30, city: "New York" };
...
这里，由于该值`country`在我们的对象中不存在`person`，因此它获得默认值`Unknown`。

另一个常见的例子是嵌套对象的解构。你可以使用另一组花括号来解构嵌套在其他对象内部的属性：
...
现在，我们来谈谈对象解构中的简写符号。创建对象时，尤其是在属性名与变量名匹配的情况下，可以使用简写语法：
```js
let name = "Bob";
let age = 25;
...
```
对象解构和简写对象表示法是强大的功能，可以使你的代码更简洁、更易于阅读。

在处理复杂数据结构或需要向函数传递多个参数时，它们尤其有用。

随着你不断使用 JavaScript，你会发现很多情况下这些技巧可以简化你的代码，使其更具表现力。
