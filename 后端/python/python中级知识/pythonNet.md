...
"""
从终端输入一个单词
以单词本中找到该单词﹐打印解释内容如果找不到则打印"找不到"
"""
with open("dict.txt","r") as f:
    input_str = input("请输入查询的单词")
...
```python
"""
编写一个文件拷贝程序﹐从终端输入一个文件﹐将文件保存在当前目录下
*文件类型不确定(可是文本文件﹐可能是二进制)
"""
filename = input("File:")
...
![3_HTTPServer](img\3_HTTPServer.png)
